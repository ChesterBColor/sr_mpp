...
#define ISP_AWB_GAIN_MIN_THR            0x100
...
static td_s32 isp_awb_rshift8(td_s32 awb_gain)
{
    td_s32 gain_shift = (awb_gain + (1 << (ISP_AWB_RIGHT_SHIFT - 1))) >> ISP_AWB_RIGHT_SHIFT;
...
static td_void awb_result_check(ot_isp_awb_result *awb_result)
{
    ...
    for (i = 0; i < OT_ISP_BAYER_CHN_NUM; i++) {
        ...
        if ((gain == 0) || (gain > ISP_AWB_GAIN_MAX_THR)) {
            isp_err_trace("awb result white_balance_gain[%d] %u should be in (%d, %d] \n",
