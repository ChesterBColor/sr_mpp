...
isp_adgamma *g_adgammat_ctx[OT_ISP_MAX_PIPE_NUM] = {TD_NULL};
#define adgamma_get_ctx(dev, ctx)   ((ctx) = g_adgammat_ctx[dev])
#define adgamma_set_ctx(dev, ctx)   (g_adgammat_ctx[dev] = (ctx))
#define adgamma_reset_ctx(dev)      (g_adgammat_ctx[dev] = TD_NULL)
...
{
    isp_agamma_attr_write(vi_pipe, &adgamma_ctx->mpi_cfg, &adgamma_ctx->mpi_cfg);
}
