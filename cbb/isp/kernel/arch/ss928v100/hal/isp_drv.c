...
td_s32 isp_drv_be_online_stt_buf_init(ot_vi_pipe vi_pipe)
{
    ...
    ret = snprintf_s(ac_name, sizeof(ac_name), sizeof(ac_name) - 1, "isp[%d].be_stt_on", vi_pipe);
    if (ret < 0) {
        return TD_FAILURE;
    }
...
static td_s32 isp_drv_be_offline_stitch_stt_malloc_buf(ot_vi_pipe vi_pipe, isp_drv_ctx *drv_ctx)
{
    ...
    ret = snprintf_s(mmb_name, sizeof(mmb_name), sizeof(mmb_name) - 1, "isp[%d].stit_be", vi_pipe);
    if (ret < 0) {
        return TD_FAILURE;
    }
...
{
    td_s8 block_id = 0; /* only support one BE */
    ...
    const ot_video_frame_info *frame_info)
{
    ...
    gap = abs_diff(drv_ctx_main_pipe->isp_sync_ctrl.last_update_pts, frame_info->video_frame.pts);
    if (gap > ISP_STITCH_MAX_GAP) {
        ...
        if ((drv_ctx_main_pipe->isp_sync_ctrl.stitch_get_cnt != 0) &&
            (drv_ctx_main_pipe->isp_sync_ctrl.stitch_get_cnt != drv_ctx->stitch_attr.stitch_pipe_num)) {
            isp_err_trace("cur pipe %d, stitch pipe num(%d), but stitch info get cnt(%d), may be discard frame!\n",
                vi_pipe, drv_ctx->stitch_attr.stitch_pipe_num, drv_ctx_main_pipe->isp_sync_ctrl.stitch_get_cnt);
        }

        drv_ctx_main_pipe->isp_sync_ctrl.last_update_pts = frame_info->video_frame.pts;
        drv_ctx_main_pipe->isp_sync_ctrl.stitch_get_cnt = 1;

        (td_void)memcpy_s(&drv_ctx->be_pre_sync_para, sizeof(isp_be_sync_para),
                          &drv_ctx->be_sync_para, sizeof(isp_be_sync_para));
        (td_void)memcpy_s(&drv_ctx->be_pre_sync_para_stitch[0], sizeof(isp_be_sync_para) * OT_ISP_MAX_PIPE_NUM,
                          &drv_ctx->be_sync_para_stitch[0], sizeof(isp_be_sync_para) * OT_ISP_MAX_PIPE_NUM);
    } else {
        drv_ctx_main_pipe->isp_sync_ctrl.stitch_get_cnt++;
    }
