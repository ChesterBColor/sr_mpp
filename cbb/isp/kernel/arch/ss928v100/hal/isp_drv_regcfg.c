...
td_void isp_drv_reg_config_vi_fpn_offline(isp_be_wo_reg_cfg *be_cfg, const isp_drv_ctx *drv_ctx)
{
    ...
    /* reset isp_fpn_en */
    for (k = 0; k < drv_ctx->work_mode.block_num; k++) {
        be_cfg->be_reg_cfg[k].pre_viproc.viproc_ispbe_ctrl0.bits.isp_fpn_en = 0;
    }
...
/*
 * adegammma enable must be same as agamma
 */
static td_void isp_drv_set_hnr_agamma_cfg(isp_viproc_reg_type *pre_viproc, isp_viproc_reg_type *post_viproc)
{
    post_viproc->viproc_ispbe_ctrl0.bits.isp_degammafe_en = pre_viproc->viproc_ispbe_ctrl0.bits.isp_pregammafe_en;
}

/*
 * snap_advanced (ref_mode_none_advanced) mode should enable gamma and disable tnr_en
 */
static td_void isp_drv_set_bnr_tnr_en_cfg(isp_viproc_reg_type *pre_viproc, isp_post_be_reg_type *post_be,
                                          isp_drv_ctx *drv_ctx)
{
    if (pre_viproc->viproc_ispbe_ctrl0.bits.isp_pregammafe_en == 1 && drv_ctx->hnr_info.hnr_en == TD_TRUE) {
        post_be->isp_bnr_cfg.bits.isp_bnr_entmpnr = 0;
    }
}

/*
 * when tnr_en disable, exchange fine_strength value
 * all hnr when isp_bnr_jnlm_gain is 0 as fine_strength is 128
 * all bnr when isp_bnr_jnlm_gain is 128 as fine_strength is 0
 */
static td_void isp_drv_set_bnr_fine_strength_cfg(isp_post_be_reg_type *post_be)
{
    const td_s32 fine_strength_max_value = 128;
    if (post_be->isp_bnr_cfg.bits.isp_bnr_entmpnr == 0 && post_be->isp_bnr_cfg7.bits.isp_bnr_b_en_ai_md == 1) {
        post_be->isp_bnr_flt_ctrl1.bits.isp_bnr_jnlm_gain_s =
            fine_strength_max_value - post_be->isp_bnr_flt_ctrl1.bits.isp_bnr_jnlm_gain_s;
        post_be->isp_bnr_flt_ctrl1.bits.isp_bnr_jnlm_gain_m =
            fine_strength_max_value - post_be->isp_bnr_flt_ctrl1.bits.isp_bnr_jnlm_gain_m;
    }
}
...
td_void isp_drv_set_hnr_bnr_cfg(isp_be_wo_reg_cfg *be_reg, isp_drv_ctx *drv_ctx, td_bool bnr_bypass, td_bool ref_none)
{
    ...
    td_bool repeat_cfg = TD_FALSE;

    if (drv_ctx->hnr_info.normal_blend == TD_TRUE && drv_ctx->hnr_info.hnr_en == TD_TRUE) {
        ai_mode = TD_TRUE;
    }

    if (drv_ctx->hnr_info.last_be_buf_vir == (td_void *)be_reg) {
        repeat_cfg = TD_TRUE;
    }
    drv_ctx->hnr_info.last_be_buf_vir = (td_void *)be_reg;
    ...
    for (i = 0; (i < drv_ctx->work_mode.block_num) && (repeat_cfg == TD_FALSE); i++) {
        /* bnr */
        isp_drv_set_hnr_bnr_snr_cfg(&be_reg->be_reg_cfg[i].post_be, drv_ctx);
        isp_drv_set_hnr_agamma_cfg(&be_reg->be_reg_cfg[i].pre_viproc, &be_reg->be_reg_cfg[i].post_viproc);
        isp_drv_set_bnr_tnr_en_cfg(&be_reg->be_reg_cfg[i].pre_viproc, &be_reg->be_reg_cfg[i].post_be, drv_ctx);
        isp_drv_set_bnr_fine_strength_cfg(&be_reg->be_reg_cfg[i].post_be);
    }
...
static td_s32 isp_drv_dynamic_blc_update_actucal_ori_info(ot_vi_pipe vi_pipe, isp_drv_ctx *drv_ctx)
{
    ...
    for (k = 0; k < drv_ctx->wdr_attr.pipe_num; k++) {
        ...
        isp_check_vir_pipe_return(vi_pipe_bind);
        isp_drv_fereg_ctx(vi_pipe_bind, fe_reg);
        actual_info->ori_black_level[k][OT_ISP_CHN_R]  = fe_reg->isp_blc_dynamic_reg4.bits.isp_blc_dynamic_value0;
        actual_info->ori_black_level[k][OT_ISP_CHN_GR] = fe_reg->isp_blc_dynamic_reg4.bits.isp_blc_dynamic_value1;
        actual_info->ori_black_level[k][OT_ISP_CHN_GB] = fe_reg->isp_blc_dynamic_reg5.bits.isp_blc_dynamic_value2;
        actual_info->ori_black_level[k][OT_ISP_CHN_B]  = fe_reg->isp_blc_dynamic_reg5.bits.isp_blc_dynamic_value3;
    }
