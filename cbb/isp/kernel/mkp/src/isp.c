...
static int isp_get_vd_end_time_out(ot_vi_pipe vi_pipe, td_u32 milli_sec, td_u32 *pu32status)
{
    ...
    drv_ctx = isp_drv_get_ctx(vi_pipe);
    if (osal_down(&drv_ctx->isp_sem_vd)) {
        return -ERESTARTSYS;
    }
...
static int isp_get_vd_be_end_time_out(ot_vi_pipe vi_pipe, td_u32 milli_sec, td_u32 *pu32status)
{
    ...
    drv_ctx = isp_drv_get_ctx(vi_pipe);
    if (osal_down(&drv_ctx->isp_sem_be_vd)) {
        return -ERESTARTSYS;
    }
...
static td_bool isp_drv_stitch_timing_err_correction(ot_vi_pipe cur_pipe, const isp_drv_ctx *drv_ctx)
{
    ...
    isp_info_trace("vi_pipe %d, cur time %llu, cfg sns last update time %llu, force to change stay_cnt frome %d->%d\n",
        cur_pipe, cur_time, drv_ctx_main->sync_cfg.last_update_time, drv_ctx_main->sync_cfg.node[0]->stay_cnt,
        drv_ctx_main->sync_cfg.node[0]->stay_cnt+1);
...
td_s32 isp_drv_get_be_sync_id(ot_vi_pipe vi_pipe, td_bool current_frm)
{
    td_s32 sync_id = -1;
    ...
    if (current_frm) {
        /* The sensor is configured one frame in advance, so plus 1 */
    ...
    while (idx <= CFG2VLD_DLY_LIMIT && drv_ctx_main->sync_cfg.node[idx] != TD_NULL) {
        if ((idx == 0) && (isp_drv_stitch_timing_err_correction(vi_pipe, drv_ctx) == TD_TRUE)) {
            stay_cnt = drv_ctx_main->sync_cfg.node[idx]->stay_cnt + 1;
        } else {
            stay_cnt = drv_ctx_main->sync_cfg.node[idx]->stay_cnt;
        }

        total_cfg_num += stay_cnt;
        if (total_cfg_num >= target_cfg_num) {
            sync_id = drv_ctx_main->sync_cfg.node[idx]->unique_id;
            goto exit;
        }
...
td_s32 isp_drv_fe_stat_buf_init(ot_vi_pipe vi_pipe)
{
    td_s32 ret, i;
    isp_drv_ctx *drv_ctx = TD_NULL;
    ...
    size = sizeof(isp_fe_stat) * MAX_ISP_FE_STAT_BUF_NUM;
    ...
    for (i = 0; i < MAX_ISP_FE_STAT_BUF_NUM; i++) {
        ...
        drv_ctx->fe_statistics_buf.node[i].stat_info.stat_key.key =
    ...
    drv_ctx->fe_statistics_buf.free_num = MAX_ISP_FE_STAT_BUF_NUM;
    ...
    isp_stat *stat, isp_stat_key stat_key)
{
    ...
    osal_list_for_each_safe(list_node, list_tmp, &drv_ctx->fe_statistics_buf.busy_list) {
        ...
        if ((g_isp_run_wakeup_sel[vi_pipe] == OT_ISP_RUN_WAKEUP_FE_START) || (fe_stat->fe_frame_pts == target_pts)) {
            isp_drv_fe_stat_mem_mov(stat, fe_stat, stat_key.bit32_isr_access);
...
td_s32 isp_drv_be_buf_exit(ot_vi_pipe vi_pipe)
{
    ...
    ret = osal_wait_event_timeout_uninterruptible(&drv_ctx->isp_exit_wait, isp_drv_wait_exit_callback,
    ...
    isp_sync_cfg_buf_node *pre_node, td_u64 *cur_sns_gain, td_u64 *pre_sns_gain)
{
    ...
    isp_drv_update_exp_ratio(sync_cfg, cur_node, exp);
}
    ...
    td_bool old_node)
{
    ...
    if ((drv_ctx->stitch_attr.stitch_enable == TD_TRUE) && (drv_ctx->stitch_attr.main_pipe == TD_FALSE)) {
        ...
        node->unique_id = (drv_ctx_main->sync_cfg.node[0] != TD_NULL) ? drv_ctx_main->sync_cfg.node[0]->unique_id : -1;
    } else {
        node->unique_id = sync_cfg->total_cnt;
        sync_cfg->total_cnt++;
        if (sync_cfg->total_cnt == 0x7FFFFFFF) {
            sync_cfg->total_cnt = 0;
        }
    }
}
...
td_s32 isp_drv_get_sync_cfg_node(ot_vi_pipe vi_pipe, isp_sync_cfg *sync_cfg, isp_sync_cfg_buf_node **cur_node_point,
    isp_sync_cfg_buf_node **pre_node_point)
{
    ...
    if (sync_cfg->node[1] != TD_NULL) {
        ...
        (td_void)memcpy_s(&sync_cfg->node_pre, sizeof(sync_cfg->node_pre),
            sync_cfg->node[0], sizeof(isp_sync_cfg_buf_node));
    }
...
td_s32 isp_drv_calc_sync_cfg(ot_vi_pipe vi_pipe, isp_sync_cfg *sync_cfg)
{
    td_s32 ret;
    td_u32 i;
    isp_sync_cfg_buf_node *cur_node = TD_NULL;
...
static td_u8 isp_drv_get_fe_sync_index_specify(ot_vi_pipe vi_pipe, const isp_drv_ctx *drv_ctx, td_bool is_first)
{
    ...
    while (idx <= CFG2VLD_DLY_LIMIT && drv_ctx->sync_cfg.node[idx] != TD_NULL) {
        total_cfg_num += drv_ctx->sync_cfg.node[idx]->stay_cnt;
        if (total_cfg_num >= target_cfg_num) {
            index = idx;
            goto exit;
        }
...
static td_u8 isp_drv_get_exp_dist_sync_index_specify(ot_vi_pipe vi_pipe, isp_drv_ctx *drv_ctx, td_bool is_first)
{
    ...
    while (idx <= CFG2VLD_DLY_LIMIT && drv_ctx->sync_cfg.node[idx] != TD_NULL) {
        if ((idx == 0) && (isp_drv_exp_stitch_timing_err_correction(vi_pipe, drv_ctx) == TD_TRUE)) {
            stay_cnt = drv_ctx->sync_cfg.node[idx]->stay_cnt + 1;
        } else {
            stay_cnt = drv_ctx->sync_cfg.node[idx]->stay_cnt;
        }
...
td_u8 isp_drv_get_exp_dist_sync_index(ot_vi_pipe vi_pipe, isp_drv_ctx *drv_ctx, td_bool is_first)
{
    td_u8 index;

    if (g_isp_run_wakeup_sel[vi_pipe] == OT_ISP_RUN_WAKEUP_BE_END) {
        index = isp_drv_get_exp_dist_sync_index_specify(vi_pipe, drv_ctx, is_first);
    } else {
        index = isp_drv_get_fe_sync_index_typical(vi_pipe, drv_ctx);
    }
...
td_u8 isp_drv_get_index(const isp_drv_ctx *drv_ctx, const td_bool low_delay_en)
{
    td_u8 index;
    ...
    if (((low_delay_en == TD_TRUE) && (g_use_bottom_half == TD_TRUE)) ||
        ((low_delay_en == TD_FALSE) && (g_use_bottom_half == TD_TRUE) &&
...
static td_void isp_dfx_sync_index_show(ot_vi_pipe vi_pipe, const isp_drv_ctx *drv_ctx)
{
    td_u8 i;
    for (i = 0; i < CFG2VLD_DLY_LIMIT; i++) {
        if (drv_ctx->sync_cfg.node[i] != TD_NULL) {
            isp_err_trace("vi_pipe = %d i = %d unique_id = %d, stay_cnt = %d\n",
                vi_pipe, i, drv_ctx->sync_cfg.node[i]->unique_id, drv_ctx->sync_cfg.node[i]->stay_cnt);
        }
    }
}
...
static td_u8 isp_drv_get_be_sync_index_specify(ot_vi_pipe vi_pipe, const isp_drv_ctx *drv_ctx)
{
    ...
    for (i = 0; i <= CFG2VLD_DLY_LIMIT; i++) {
        if (drv_ctx->sync_cfg.node[i] != TD_NULL && drv_ctx->sync_cfg.node[i]->unique_id == sync_unique_id) {
            isp_info_trace("vi_pipe = %d, target sync unique_id = %d, find match index = %d\n",
                vi_pipe, sync_unique_id, i);
            return i;
        }
    }

    if (i == CFG2VLD_DLY_LIMIT + 1) {
        isp_err_trace("vi_pipe = %d, target sync unique_id = %d, can't find match index! pts diff = %llu us\n",
            vi_pipe, sync_unique_id, call_sys_get_time_stamp() - drv_ctx->sync_cfg.cur_pts);
...
static td_u32 isp_get_sensor_cfg_index(ot_vi_pipe vi_pipe, const isp_drv_ctx *drv_ctx)
{
    ...
    if (g_isp_run_wakeup_sel[vi_pipe] == OT_ISP_RUN_WAKEUP_FE_START ||
        (drv_ctx->sync_cfg.node[0] == TD_NULL) || (drv_ctx->sync_cfg.node[0]->stay_cnt != 0)) {
        return 0;
    }

    for (i = 0; i < CFG2VLD_DLY_LIMIT - 1; i++) {
        if ((drv_ctx->sync_cfg.node[i] != TD_NULL) && (drv_ctx->sync_cfg.node[i + 1] == TD_NULL)) {
            break;
        } else if ((drv_ctx->sync_cfg.node[i] != TD_NULL) && (drv_ctx->sync_cfg.node[i]->stay_cnt == 0) &&
            (drv_ctx->sync_cfg.node[i + 1] != TD_NULL) && (drv_ctx->sync_cfg.node[i + 1]->stay_cnt != 0)) {
            break;
        }
    }
...
td_void isp_drv_get_exp_ratio(ot_vi_pipe vi_pipe, const isp_drv_ctx *drv_ctx, td_u32 *ratio)
{
    ...
    td_u8 cur_idx, isp_sync_cfg_buf_node *cur_node)
{
    ...
    next_node = (drv_ctx->sync_cfg.node[next_idx] == TD_NULL) ? cur_node : drv_ctx->sync_cfg.node[next_idx];

    isp_drv_calc_sns_gain(&drv_ctx->sync_cfg, cur_node, pre_node, cur_sns_gain, pre_sns_gain);

    /* calculate exposure ratio */
    isp_drv_calc_exp_ratio(&drv_ctx->sync_cfg, cur_node, pre_node, cur_sns_gain, pre_sns_gain);
...
td_s32 isp_drv_reg_config_isp_be(ot_vi_pipe vi_pipe)
{
    ...
    if (drv_ctx->sync_cfg.vc_cfg_num == cfg_node_vc) {
        ...
        isp_drv_get_exp_ratio(vi_pipe, drv_ctx, ratio);
...
td_void isp_drv_update_be_sync_para_by_id(ot_vi_pipe vi_pipe, const ot_video_frame_info *frame_info)
{
    td_s32 ret;
    isp_drv_ctx *drv_ctx = TD_NULL;
    ot_video_supplement_misc *misc_info = TD_NULL;
    td_s32 sync_id = -1;
    ...
    misc_info = (ot_video_supplement_misc *)frame_info->video_frame.supplement.misc_info_virt_addr;
    if (misc_info != TD_NULL) {
        sync_id = (td_s32)((misc_info->private_data[1] >> 32) & 0xFFFFFFFF); /* 32: high bit */
    } else {
...
isp_sync_cfg_buf_node *isp_drv_get_sns_cfg_node(isp_drv_ctx *drv_ctx, td_u8 cur_node_idx, td_u8 delay_frm_num)
{
...
static td_bool is_sensor_data_has_been_config(isp_sync_cfg_buf_node *cfg_node, td_u32 idx, td_u8 stay_cnt)
{
...
td_s32 isp_drv_write_ssp_data(isp_drv_ctx *drv_ctx, td_u8 cur_node_idx, td_u32 update_pos, td_s8 ssp_dev,
    td_s8 ssp_cs, td_u8 stay_cnt)
{
    td_u32 i;
    td_u8 cfg_node_id;
    isp_sync_cfg_buf_node *cur_node = drv_ctx->sync_cfg.node[cur_node_idx];
...
td_s32 isp_drv_reg_config_sensor(ot_vi_pipe vi_pipe, isp_drv_ctx *drv_ctx)
{
    ...
    isp_drv_ctx *drv_ctx_main = TD_NULL;
    isp_sync_cfg_buf_node *cur_node = TD_NULL;
    isp_sync_cfg_buf_node *cur_node_valid = TD_NULL;
    ...
    if (drv_ctx->stitch_attr.stitch_enable == TD_TRUE && drv_ctx->stitch_attr.main_pipe == TD_FALSE) {
        drv_ctx_main = isp_drv_get_ctx(drv_ctx->stitch_attr.stitch_bind_id[0]);
        cfg_node_idx = isp_get_sensor_cfg_index(drv_ctx->stitch_attr.stitch_bind_id[0], drv_ctx_main);
        cur_node_valid = drv_ctx_main->sync_cfg.node[cfg_node_idx];
        if (cur_node_valid == TD_NULL) {
            isp_warn_trace("cur_node NULL pointer Stitch!\n");
            return TD_FAILURE;
        }
    } else {
        drv_ctx_main = drv_ctx;
        cur_node_valid = cur_node;
    }
    ...
    if (cur_node->sns_regs_info.sns_type == OT_ISP_SNS_I2C_TYPE) {
        ...
        ret = isp_drv_write_ssp_data(drv_ctx_main, cfg_node_idx, drv_ctx->int_pos, ssp_dev, ssp_cs, stay_cnt);
...
td_s32 isp_drv_be_end_int_proc(ot_vi_pipe vi_pipe)
{
    ...
    if (g_isp_run_wakeup_sel[vi_pipe] == OT_ISP_RUN_WAKEUP_BE_END) {
        isp_drv_run_trigger_prepare(vi_pipe, drv_ctx);
    }
...
td_s32 isp_drv_proc_exit(ot_vi_pipe vi_pipe)
{
    ...
    // it is necessary to up semphore firstly when user process was killed(maybe semphore is down)
    if (drv_ctx->proc_sem_flag == TD_TRUE) {
        osal_up(&drv_ctx->proc_sem);
        drv_ctx->proc_sem_flag = TD_FALSE;
    }
...
td_void isp_cal_sync_info_index(ot_vi_pipe vi_pipe, td_s32 *index)
{
    ...
    *index = clip3(idx, 0, CFG2VLD_DLY_LIMIT - 1);
}
...
static td_void isp_get_actual_exp_distance(ot_vi_pipe vi_pipe, ot_isp_frame_info *isp_frame, isp_drv_ctx *drv_ctx)
{
    ...
    isp_sync_cfg_buf_node *cfg_node = TD_NULL;
    td_u32 delay_frm_num = 0;

    fe_cfg_node_idx = isp_drv_get_exp_dist_sync_index(vi_pipe, drv_ctx, TD_FALSE);
    ...
    // Take the specific distance according to exp_distance
    delay_frm_num = cfg_node->sns_regs_info.distance_attr.delay_frame_num;
    fe_cfg_node_idx += delay_frm_num;
    fe_cfg_node_idx = MIN2(fe_cfg_node_idx, CFG2VLD_DLY_LIMIT - 1);
    cfg_node = drv_ctx->sync_cfg.node[fe_cfg_node_idx];
    if (cfg_node == TD_NULL) {
        return;
    ...
    for (i = 0; i < (OT_ISP_WDR_MAX_FRAME_NUM - 1); i++) {
        isp_frame->exposure_distance[i] = cfg_node->sns_regs_info.distance_attr.exp_distance[i];
    }
}
...
td_s32 isp_get_actual_frame_info(ot_vi_pipe vi_pipe, ot_isp_frame_info *isp_frame)
{
    ...
    if ((drv_ctx->mem_init == TD_FALSE) && (drv_ctx->wdr_attr.wdr_mode == OT_WDR_MODE_NONE)) {
        for (vi_pipes = 0; vi_pipes < OT_ISP_MAX_PIPE_NUM; vi_pipes++) {
            ...
            isp_get_actual_exp_distance(vi_pipe, isp_frame, drv_ctx_s);
            osal_spin_unlock_irqrestore(&g_isp_lock[vi_pipe], &flags);
        }
    } else {
#ifdef CONFIG_OT_SNAP_SUPPORT
...
td_s32 isp_drv_get_wdr_attr(ot_vi_pipe vi_pipe, vi_pipe_wdr_attr *wdr_attr)
{
    ...
    if ((is_half_wdr_mode(wdr_attr_str.wdr_mode) == TD_TRUE) &&
        (g_isp_run_wakeup_sel[vi_pipe] == OT_ISP_RUN_WAKEUP_BE_END)) {
...
td_s32 isp_drv_reset_ctx(ot_vi_pipe vi_pipe)
{
    ...
    drv_ctx->hnr_info.last_be_buf_vir = TD_NULL;
    ...
    *find_flag = TD_TRUE;
    ...
    switch (ioctl_para->cmd) {
        ...
        case ISP_PROC_WRITE_ING: {
            ...
            g_isp_drv_ctx[vi_pipe].proc_sem_flag = TD_TRUE;
            return TD_SUCCESS;
        }

        case ISP_PROC_WRITE_OK: {
            osal_up(&g_isp_drv_ctx[vi_pipe].proc_sem);
            g_isp_drv_ctx[vi_pipe].proc_sem_flag = TD_FALSE;
            return TD_SUCCESS;
        }
...
static void isp_drv_work_queue_handler(struct osal_work_struct *worker)
{
    ...
    if (osal_down(&isp_work_queue->sem)) {
        return;
    }
...
td_s32 isp_drv_int_status_process(ot_vi_pipe vi_pipe, td_s32 vi_dev, isp_drv_ctx *drv_ctx, td_u32 int_status)
{
    ...
    td_u32 be_pre_int, wch_int_mask;
    ...
    if (drv_ctx->work_mode.running_mode == ISP_MODE_RUNNING_PRE_ONLINE_POST_OFFLINE) {
        be_pre_int = isp_drv_get_pre_proc_fstart_status();
