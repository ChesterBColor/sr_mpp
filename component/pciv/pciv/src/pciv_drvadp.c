...
td_s32 pciv_drv_adp_send_msg(const ot_pciv_remote_obj *remote_obj, pciv_msg_type type, const pciv_pic *recv_pic)
{
    ...
    if (remote_obj->chip_id > VENDOR_MAX_MAP_DEV || remote_obj->chip_id < 0) {
        pciv_err_trace("target(%d) is out of range [0, %d]!\n", remote_obj->chip_id, VENDOR_MAX_MAP_DEV);
        return TD_FAILURE;
    }
...
td_s32 pciv_drv_adp_msg_isr(td_void *handle, td_void *buf, td_u32 data_len)
{
    ...
    if (g_local_id != msg->target) {
        if (g_local_id != 0) {
            ...
            pciv_err_trace("who are you? target id(%u)\n", msg->target);
            return 0;
        }
    ...
    switch (msg->msg_type) {
        ...
        default: {
            pciv_err_trace("Unknown message type:%u\n", msg->msg_type);
            break;
        }
    }
...
td_s32 pciv_drv_adp_get_window_base(ot_pciv_window_base *win_base)
{
    ...
    if (win_base->chip_id < 0 || win_base->chip_id > VENDOR_MAX_MAP_DEV) {
        pciv_err_trace("pcie device id(%d) is out of range [0, %d].\n", win_base->chip_id, VENDOR_MAX_MAP_DEV);
    ...
    if (ret != TD_SUCCESS) {
        pciv_err_trace("unknown pcie device id(%d)\n", win_base->chip_id);
    }
    return ret;
}
...
static td_s32 pciv_drv_adp_host_init(ot_pciv_window_base *win_base, int n_remot_id[])
{
    ...
    for (i = 0; i < VENDOR_MAX_MAP_DEV; i++) {
        if (n_remot_id[i] != PCIV_DRV_INVALID_CHIP_ID) {
            ...
            if (ret != 0) {
                pciv_err_trace("host check slave chip(%d) failed\n", n_remot_id[i]);
                return TD_FAILURE;
            }
            ...
            if (g_msg_handle[n_remot_id[i]] == TD_NULL) {
                pciv_err_trace("os_mcc_open chip(%d) failed\n", n_remot_id[i]);
                return TD_FAILURE;
            }
...
static td_s32 pciv_drv_adp_slave_init(ot_pciv_window_base *base_win)
{
    ...
    if (g_msg_handle[0] == TD_NULL) {
        pciv_err_trace("can't open mcc device 0!\n");
        return TD_FAILURE;
    }
...
td_s32 pciv_drv_adp_init(td_void)
{
    ...
    if (g_chip_cnt > VENDOR_MAX_MAP_DEV) {
        pciv_err_trace("The chip count %d is out of range [%d, %d]!\n", g_chip_cnt, 1, VENDOR_MAX_MAP_DEV);
        return TD_FAILURE;
    }
    ...
    if (n_remot_id[0] == 0) {
        ...
        if (ret != 0) {
            pciv_err_trace("slave check host failed!\n");
            return TD_FAILURE;
        }
    }
