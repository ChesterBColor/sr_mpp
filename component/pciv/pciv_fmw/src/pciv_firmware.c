...
td_s32 pciv_firmware_reset_chn_queue(ot_pciv_chn pciv_chn)
{
    ...
    if (fmw_chn->is_start == TD_TRUE) {
        pciv_fmw_spin_unlock(flags);
        pciv_fmw_err_trace("chn%d has not been stoped, please stop it first!\n", pciv_chn);
    ...
    for (i = 0; i < busy_num; i++) {
        ...
        if (node == TD_NULL) {
            pciv_fmw_spin_unlock(flags);
            pciv_fmw_err_trace("chn%d get busy node failed!\n", pciv_chn);
            return OT_ERR_PCIV_NOT_PERM;
        }
...
td_s32 pciv_firmware_create(ot_pciv_chn pciv_chn, const ot_pciv_attr *attr, td_s32 local_id)
{
    ...
    if (fmw_chn->is_create == TD_TRUE) {
        pciv_fmw_err_trace("chn%d has been created\n", pciv_chn);
...
td_s32 pciv_firmware_start(ot_pciv_chn pciv_chn)
{
    ...
    fmw_chn = pciv_fmw_get_context(pciv_chn);
    if (fmw_chn->is_create != TD_TRUE) {
        pciv_fmw_err_trace("chn%d has not been created\n", pciv_chn);
...
td_s32 pciv_firmware_window_vb_create(const ot_pciv_window_vb_cfg *cfg)
{
    ...
    for (i = 0; i < cfg->pool_cnt; i++) {
        ...
        if (ret != TD_SUCCESS) {
            pciv_fmw_alert_trace("create pool index(%u), cnt(%d), size(%d) failed\n",
...
td_s32 pciv_firmware_malloc(td_u32 size, td_s32 local_id, td_phys_addr_t *phys_addr)
{
    ...
    if (size == 0) {
        pciv_fmw_err_trace("malloc fail, size:%d can not be 0!\n", size);
    ...
    td_phys_addr_t *phys_addr)
{
    ...
    if (index >= OT_PCIV_MAX_BUF_NUM) {
        pciv_fmw_err_trace("the buffer index(%u) is out of range [0, %d]!\n", index, OT_PCIV_MAX_BUF_NUM - 1);
        return OT_ERR_PCIV_ILLEGAL_PARAM;
    }
    if (size == 0) {
        pciv_fmw_err_trace("call_vb_get_blk_by_size fail, size:%d can not be 0!\n", size);
        return OT_ERR_PCIV_ILLEGAL_PARAM;
    }
    pciv_fmw_spin_lock(flags);
    if (local_id != 0) {
...
td_s32 pciv_firmware_free_chn_buffer(ot_pciv_chn pciv_chn, td_u32 index, td_s32 local_id)
{
    ...
    if (index >= OT_PCIV_MAX_BUF_NUM) {
        pciv_fmw_err_trace("the index(%u) is out of range [0, %d]!\n", index, OT_PCIV_MAX_BUF_NUM - 1);
        return OT_ERR_PCIV_ILLEGAL_PARAM;
    }
    ...
    for (i = 0; i < fmw_chn->count; i++) {
        ...
        if (g_pciv_fmw_call_back.pf_recv_pic_free) {
            ...
            if (ret != OT_ERR_PCIV_BUF_EMPTY) {
                pciv_fmw_err_trace("chn%d pf_recv_pic_free failed.\n", pciv_chn);
                continue;
            }
        }
    }
...
td_s32 pciv_firmware_recv_pic_and_send(ot_pciv_chn pciv_chn, const pciv_pic *recv_pic)
{
    ...
    if (recv_pic->index >= OT_PCIV_MAX_BUF_NUM) {
        pciv_fmw_err_trace("the buffer index(%u) is out of range [0, %d]!\n", recv_pic->index, OT_PCIV_MAX_BUF_NUM - 1);
        return TD_FAILURE;
    }
    ...
    fmw_chn = pciv_fmw_get_context(pciv_chn);
    if (fmw_chn->is_start != TD_TRUE) {
        pciv_fmw_err_trace("chn%d has not been started!\n", pciv_chn);
        pciv_fmw_spin_unlock(flags);
    ...
    /*
     * when start timer, PcivFirmWareVoPicFree has not lock synchronous
    ...
    const ot_video_frame_info *video_frame)
{
    ...
    pciv_fmw_spin_lock(flags);
    if (fmw_chn->is_start != TD_TRUE) {
        pciv_fmw_info_trace("chn%d has been stopped\n", pciv_chn);
        pciv_fmw_spin_unlock(flags);
        goto out;
    }
    pciv_fmw_spin_unlock(flags);
    ...
    if (ret != TD_SUCCESS) {
        pciv_fmw_err_trace("add user vb failed for chn%d!\n", pciv_chn);
        goto out;
    }
    ...
    if (ret != TD_SUCCESS) {
        ...
        pciv_fmw_info_trace("chn%d pf_src_send_pic failed!\n", pciv_chn);
        goto out;
    }
    ...
    const vgs_task_data *vgs_task)
{
    ...
    if (fmw_chn->is_start != TD_TRUE) {
        pciv_fmw_info_trace("chn%d has not been stopped\n", pciv_chn);
        return TD_FAILURE;
    }
    ...
    if (vgs_task->finish_stat != VGS_TASK_FNSH_STAT_OK) {
        pciv_fmw_err_trace("chn%d vgs task finish status is not ok\n", pciv_chn);
        return TD_FAILURE;
    }
    return TD_SUCCESS;
}
    ...
    ot_vb_calc_cfg *vb_calc_cfg, const ot_video_frame_info *src_frame)
{
    ...
    if (ret != TD_SUCCESS) {
        pciv_fmw_err_trace("chn%d pciv_fmw get vgs vb_calc_cfg failed!\n", pciv_chn);
        return ret;
    }

    *vgs_task = pfn_vgs_exp_func->pfn_vgs_get_free_task();
    if (*vgs_task == TD_NULL) {
        pciv_fmw_err_trace("chn%d pciv get task failed\n", pciv_chn);
        return TD_FAILURE;
    }

    ret = pciv_fmw_get_vgs_out_vb(pciv_chn, vb_handle);
    if (ret != TD_SUCCESS) {
        pciv_fmw_err_trace("chn%d pciv_fmw get vgs out_put VB failed!\n", pciv_chn);
        goto free_task;
    }
    ...
    if (ret != EOK) {
        pciv_fmw_err_trace("chn%d copy frame info to img_in failed!\n", pciv_chn);
        goto free_task;
    }
    ...
    if (ret != EOK) {
        pciv_fmw_err_trace("chn%d copy frame info to img_out failed!\n", pciv_chn);
        goto free_task;
    }
    return TD_SUCCESS;
free_task:
    ...
    const ot_video_frame *out_frame, pciv_fmw_vgs_task *fmw_task)
{
    ...
    if (ret != TD_SUCCESS) {
        pciv_fmw_err_trace("chn%d add user vb failed!\n", pciv_chn);
        return ret;
    }
    return TD_SUCCESS;
}
...
static td_void pciv_fmw_src_vgs_task_cb(ot_mod_id call_mod_id,
    td_s32 call_dev_id, td_s32 call_chn_id, const vgs_task_data *vgs_task)
{
    td_s32                      ret;
    td_ulong                    flags;
    ot_pciv_chn                 pciv_chn;
    ...
    const vgs_online_opt *vgs_opt)
{
    ...
    if (ret != TD_SUCCESS) {
        ...
        pciv_fmw_err_trace("chn%d begin vgs job failed!\n", pciv_chn);
        pfn_vgs_exp_func->pfn_vgs_put_free_task(vgs_task);
        return TD_FAILURE;
    }
    ...
    if (ret != TD_SUCCESS) {
        ...
        pciv_fmw_err_trace("chn%d create vgs task failed!\n", pciv_chn);
    ...
    if (ret != TD_SUCCESS) {
        ...
        pciv_fmw_err_trace("chn%d end vgs job failed!\n", pciv_chn);
        pfn_vgs_exp_func->pfn_vgs_cancel_job(vgs_handle);
        return TD_FAILURE;
    }
    ...
    if (g_drop_err_timeref == 1) {
        ...
        if ((obj->type == PCIV_BIND_VI) || (obj->type == PCIV_BIND_VO)) {
            if (pic_info->video_frame.time_ref < fmw_chn->time_ref) {
                pciv_fmw_err_trace("chn%d, time_ref err, (%d, %d)\n",
...
static td_s32 pciv_send_vdec_pic_timer_state_check(ot_pciv_chn pciv_chn, pciv_fmw_chn *fmw_chn)
{
    ...
    if (fmw_chn->send_state == PCIVFMW_SEND_OK) {
        ...
        if (fmw_chn->cur_vdec_node == TD_NULL) {
            pciv_fmw_info_trace("chn%d has no vdec pic\n", pciv_chn);
            return TD_FAILURE;
        }
        return TD_SUCCESS;
    }
    ...
    pciv_fmw_info_trace("chn%d send vdec pic error, state %d\n", pciv_chn, fmw_chn->send_state);
    return TD_FAILURE;
}
...
static td_void pciv_send_vdec_pic_timer_work(ot_pciv_chn pciv_chn, pciv_fmw_chn *fmw_chn)
{
    ...
    if (fmw_chn->is_master == TD_TRUE) {
        ...
        pciv_fmw_warn_trace("chn%d, start: %d, start send pic to other mod.\n", pciv_chn, fmw_chn->is_start);
        ...
        if ((ret != TD_SUCCESS) &&
            ...
            pciv_fmw_warn_trace("chn%d, start: %d, finish send pic to other mod.\n",
                pciv_chn, fmw_chn->is_start);
            pciv_fmw_info_trace("chn%d send pic failed, put to queue and send again.\n", pciv_chn);
    ...
    td_bool is_block, const ot_video_frame_info *frame_info)
{
    ...
    if (node == TD_NULL) {
        pciv_fmw_debug_trace("chn%d has no free node\n", pciv_chn);
        pciv_fmw_spin_unlock(flags);
        return TD_FAILURE;
    }
    ...
    td_void *pic_info)
{
    ...
    if (fmw_chn->is_start != TD_TRUE) {
        pciv_fmw_spin_unlock(flags);
        pciv_fmw_info_trace("chn%d has not stopped\n", pciv_chn);
        return TD_FAILURE;
    }
    fmw_chn->get_cnt++;
    pciv_fmw_spin_unlock(flags);
    ...
    const vpss_query_info *query_info, const pciv_fmw_chn *fmw_chn, vpss_inst_info *inst_info)
{
    ...
    if (vb_handle == OT_VB_INVALID_HANDLE) {
        ...
        pciv_fmw_alert_trace("get VB for pic_size: %d from mmz:%s fail, for chn%d VPSS query\n",
...
send_fail:
    ...
    g_pciv_fmw_call_back.pf_free_pciv_chn_share_buf_state(chn_id);
#endif
    pciv_fmw_alert_trace("chn%d deal with vpss send pic failed!\n", pciv_chn);
    return ret;
}
...
td_s32 pciv_fmw_reset_call_back(td_s32 dev_id, td_s32 chn_id, td_void *pv_data)
{
    ...
    for (i = 0; i < busy_num; i++) {
        ...
        if (ret != TD_SUCCESS) {
            pciv_fmw_alert_trace("chn%d call_vb_user_sub failed!\n", pciv_chn);
        }
